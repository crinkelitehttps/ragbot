...
constexpr std::array<uint32_t, 64> k_table = {
...
void compress(std::array<uint32_t, 8>& state, const uint8_t block[64])
{
    ...
    for (int i = 0; i < 64; ++i) {
        ...
        const uint32_t t1 = h + S1 + ch + k_table[i] + w[i];
...
std::array<uint8_t, 32> sha256_raw(const uint8_t* data, std::size_t len)
{
    ...
    std::size_t off = 0;
    while (off + 64 <= len) { compress(state, data + off); off += 64; }
    uint8_t tail[128] = {0};
    const std::size_t rem = len - off;
    std::memcpy(tail, data + off, rem);
    tail[rem] = 0x80;
    const std::size_t pad_target = (rem < 56) ? 64 : 128;
    for (int i = 0; i < 8; ++i) {
        tail[pad_target - 1 - i] = static_cast<uint8_t>((total_bits >> (i * 8)) & 0xFF);
    }
    compress(state, tail);
    if (pad_target == 128) compress(state, tail + 64);
...
{
    bool ok = false;
    const Bytes bytes = read_file_bytes(path, &ok);
    if (!ok) return {};
